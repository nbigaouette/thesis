...
\fxnote[noinline,margin]{Finish $V_b$ section}
\fxnote[noinline,margin]{Create real $V_b$ figure}
...
\emptyfig{Cluster potential and the $V_b$ approximation}{fig:md:vb}
...
\fxfatal{Add the divergence calculation here. See hand notes.}
...
\fxnote[margin,noinline]{Find a better name for section \ref{section:intro:md:implementation}}

Implementing any kind of ionization in the model is done through a
...
easily integrated into the model. Rates are obtained using
\begin{align}
\Gamma\pa{\omega} = \sigma\pa{\omega} I\pa{t}.
\label{eqn:ionization:rate}
\end{align}
for all processes, including single photon ionization (equivalent to equation
\eqref{eqn:ionization:rate:mpi} with $\nu = 1$).


For the present work, Xenon single-photon ionization experimental cross-sections
$\sigma\pa{\omega}$ were obtained from reference \cite{West1978} and from
reference \cite{Marr1976} for Argon. \fxfatal{Add source of impact ionization cross
sections}
