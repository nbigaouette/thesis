...
\fxnote[noinline,margin]{Is that the only reason for rare-gas studies? Reference?}
...
\fxnote[noinline,margin]{See Long2010\cite{Long2010} for what happens when $\gamma \approx 1$}
...
\fxfatal[margin,noinline]{38 nm or 32 nm?? Bosted says 38, but Eddie says 32!
                          See HHG section where 38 is used.}
% http://photon-science.desy.de/news__events/research_highlights/archive/flash_excites_giant_atomic_resonance/index_eng.html
\fxnote[margin,noinline]{I want to show how deep we can reach with photons in
                         XUV and VUV. Which shells? Resonance?}
\fxnote[margin,noinline]{4d ionization: single or two photons?}
...
\end{center}
\end{table}
\fxnote[margin,noinline]{Even though the 4d's Ip is large, the resonance makes
                         it accessible. See recombination paper and talk about
                         it here.}

Let's now look at the different mechanisms present in the different regimes.
...
\fxnote[noinline,margin]{Create real tunnel ionization figure}
\begin{figure}
 \centering
...
\fxnote[margin,noinline]{Cite an HHG review reference}
...
\fxfatal{Add how IBH scales with frequency. Landau \& Lifshitz? Couldn't find it...}
\fxfatal{Cite Schlessinger1979\cite{Schlessinger1979} for IBH parameters at different regimes}
...
\fxnote[noinline,margin]{Create real single-photon ionization figure}
\begin{figure}
 \centering
...
\emptyfig{Auger effect}{fig:auger}
\fxfatal[margin,noinline]{Create Auger figure}
...
\fxnote[noinline,margin]{Create real impact ionization figure}
\begin{figure}
 \centering
...
\fxnote[noinline,margin]{Create real heating figures}
...
\fxwarning[margin,noinline]{Really include this paragraph?}
...
% The rest of the thesis is presented as a series of published or draft articles.
% The first article in section \ref{section:papers:aci} presents the article
...
This work on excited states in clusters was first published in the article
``Augmented collisional ionization via excited states in XUV cluster
interactions'' published in 2011 in \textit{Journal of Physics B: Atomic,
...
\fxfatal[noinline,margin]{Synchronize the 100nm paper with its description}
...
2002 DESY-FEL experiment intensity was re-calibrated in 2010 from
$\ten{2}{13}$~W/cm$^2$ to $\ten{8}{12}$~W/cm$^2$, we hypothesized that our model
ACI could still explain the high charge states seen in xenon clusters, even
with the lower intensities. We included single photon ionization, impact
ionization, ACI and recombination in simulations of Xe$_{90}$ to Xe$_{5083}$
interacting with a 100 fs, 98 nm (12.65 eV) laser pulse. We compared our model
with previous work with ACI disabled and found good agreement. Since photons
cannot ionize a Xe$^{1+}$ to Xe$^{2+}$ (see table \ref{tab:ips} for xenon
ionization potentials), all charge states higher than Xe$^{1+}$ must be created
with other processes, in agreement with experiments. The high performance of the
...
charge states and the highest charge states matched.

The last part of the article discuss the influence of the potential depth
of equation \eqref{eqn:md:sigma}. Santra and Greene suggested\cite{Greene2003}
using atomic potentials instead of a pure Coulombic one to explain the high
charge states seen in experiments. By allowing deeper potentials in our
...
\fxfatal[noinline,margin]{Add in the 100nm paper CRP in the model validation section}
