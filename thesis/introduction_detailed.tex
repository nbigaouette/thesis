...
properties where their size could be controlled to produce different glass colors.
...
\begin{itemize}
\item Cluster studies done for many years
\item Shown interesting phenomena
    \begin{itemize}
    \item Enhanced absorption
    \item Nuclear processes
    \item Others
    \item Etc.
    \end{itemize}
\item Interesting because:
    \begin{itemize}
    \item Bridge the gap between single atom and solids
    \item Test of light-matter interaction
    \item Explosions products accessible
        \begin{itemize}
        \item Not the case with solids/bulks
        \item Provides detailed info about dynamics
        \end{itemize}
    \item Numerical studies possible
    \end{itemize}
\item Light sources moving towards lower wavelengths
    \begin{itemize}
    \item What is important at these wavelengths?
    \item Talk about X-rays here, but not too much
    \item Experiments at DESY
    \item Experiments at SLAC
    \end{itemize}
\end{itemize}


As the Roman saw, the size of clusters is a key properties.
...
will \cite{Martin1996}. Because of their scalability\cite{Reinhard2004}, they
are the perfect test-bed for the study of nanoscale objects. New coherent light
sources, in this case FEL, now gives us the needed tools to probe these. While
the infrared (IR) regime has been thoroughly studied\cite{Fennel2010}, shorter
wavelengths -- Vacuum Ultra-Violet (VUV), Extreme Ultra-Violet (XUV), soft
X-Rays, etc. -- are still relatively unexplored fields.
...
\subsubsubsection{Inverse Bremsstrahlung}
