...
Lately, laser light sources have been moving to shorter wavelengths; Vacuum
Ultra-Violet (VUV), Extreme Ultra-Violet (XUV), soft X-Rays and even hard X-rays.
In Free Electron Lasers (FEL), relativistic electrons are sent through an
ondulator in which they emit a coherent pulse, tunable in wavelength from
microwaves to X-rays\cite{Brabec2009,Ackermann2007a} at high intensities.

At shorter wavelengths, the ponderomotive force of the laser field on electrons
decreases as it is inversely proportional to the laser frequency (squared).
Electrons will thus not have a large quiver motion as opposed to the IR regime
and the proportion of the different energy transfer mechanisms from the laser
to the cluster changes. For example, tunnel ionization disappear while single
photons can now directly ionize atoms or even ions.
...
are still relatively unexplored fields.
