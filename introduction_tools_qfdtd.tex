...
FDTD is an %(old)
established algorithm developed in 1966 by Yee\cite{Yee1966} to solve
...
% i heard there is an updated fdtd version
...
Both real-time and imaginary-time methods were implemented for this thesis. Let's describe the
two algorithms.
...
Further, a Wick rotation is performed in time ($\im t
...
the different eigenstates from each other. I developed a novel method to isolate these
states, as described in the published article of section
\ref{section:papers:qfdtd} (page \pageref{section:papers:qfdtd}).

% I changed the wording a bit above; also describe briefly why you needed to develop a novel method to isolate those states
...
In this thesis I developed a method to
%novel method was developed to
concentrate three dimensional grid cells close
to centres of interest and thus reduce significantly the amount of memory
