...
\lrfatal{somewhere you should mention that the order (N^2) scaling is only for
...
% you may also want to mention somewhere that we are treating ions and electrons, and that ions have different charge states possible. and that we choose our ion potentials so that they scale - so that a +3 ion + electron on top looks like a +2, for example.
...
% need to have much better flow to the next section. an introductory paragraph of something. It's really jarring. perhaps move the MD implementation into the previous section somehow, and make the below on the implementation details of the quantum rates??


\subsubsection{Potential threshold $V_b$}
\label{section:intro:Vb}

Many ionization processes described in the introduction %previously
...
% this is *not true* within the cluster environment, but only in the isolated atom picture. in the cluster environment, don't we use Vb?
...
$Z/r$.
% refer to what it really is, with the softened potentials!
%
...
% stress that this zero at infinity is only for the isolated atom case, and that in the cluster environment it's influenced by Vb. because except for the very first ionization (where all are neutral, so there is no Vb), this will NOT be true for a cluster.
...
% please clarify that the above and below paragraphs are only for the isolated atom case
...
% you might also want to mention that we use some calculated and some experimental cross sections.
...
%some changes in the below, not indictated directly...
...
% Nic: verify this is true - I think 'ionization potential' is very unclear here.
recombination is forced: the electron disappears from the simulation and the ion
charge state is increased by one.

%Revise the below to include the recomb paper:
