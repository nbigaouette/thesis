...
% any citations you can add to the above?
...
% make reference to attosecond science, rare gas atoms, and provide a reference to a book or review article - I moved the paragraph below to this spot. you'll have to clean it up a bit, with regards to the definition of "infrared"
...
% maybe add the icosohedral reference in the above paragraph instead?
...
%For these reasons, clusters are invaluable tools to study light and matter interaction. 
% .. the above sentence is almost found word for word in a previous paragraph
...
in the IR regime\cite{Fennel2010}. 
Varin \textit{et al.}\cite{Varin2012}
...
% i took out the paragraph on atto here... thought it would be better above.
...
diffraction patterns where obtained for clusters in single laser shots.%wavelength?
...
%(any more recent references from flash or lcls?? there must be...

%... and there is much theoretical work to be done.
...
laser
%
% this is not necessarily true. you can say they can affect energy absorption and/or charge state spectra
%
...
%
will be brought from %?? I don't understand this sentence, especially what you mean from this phrase
%
...
% you should define what you mean by quivery energy

Note that the two regimes are not clearly segregated; Keldysh parameter
%states which kind of processes dominates but does not prevent them from happening.
...
% i made the change to the last sentence, but am still not happy with it. it's b/c the laser field picture is really only a semiclassical quantum picture in the length gauge. i will clarify more when we talk and maybe we can edit it appropriately then.
...
%When the laser polarization changes,
...
\cite{Lewenstein2008}.
% you should be prepared to answer "why only odd harmonics?"
...
%
% relate this to the "excursion" length, which is related to quiver energy, and the cluster size to give an intuitive picture of why.
%
...
%xuv is a different regime from vuv. to clarify, give the expected ranges for both regimes from a reliable source
...
% there is also the above-threshold ionization regime, which you might want to mention, and was one of the first discoveries in the atto field
...
% i think this description of auger is good enough to include the other paper. then just remove the citation to the paper from ehre.
...
%fragments - fragments usually means two or more atoms still bound together
ions
up to
...
% nic, did we ever talk about the downsides of the rost model?
...
% the above is akward, please reword...
...
% we didn't "decide" to not use a quantum picture, we "couldn't". please reword.

% also remind me to talk to you about justifications of classical regmies in plasma physics... we might add that discussion here.
...
% if we add the recomb paper, do we have to change or add to the above?
...
% your contributions to this paper.
