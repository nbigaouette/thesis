...
\lrnote[noinline,margin]{for the below, want to emphasize that microscopic pic
is a NEW method, rather than the estabilished methods you describe above. and you
could say that it is an interesting future direction, or something like that.}
PIC methods, where particles interact with a grid which propagates the
...
larger number of particles, for smaller systems the grid adds
an extra overhead not present in MD.

\lrnote{add citation for MD method. might also mention that there are
refinements on md, like treecode, and also the limitation that it cannot account
for field propagation effects, like the microscopic pic can.}
...
\lrnote[noinline,margin]{It is hartree-fock, isn't it? just want to verify}
